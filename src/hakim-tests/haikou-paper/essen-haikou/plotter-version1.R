# $Id: plotter-version1.R 2012-05-28 10:34:55Z ctdao $
# ###########################################################################
                     ...
                     bLevels              = 1)
{
   ...
   # ------ Create plot window ----------------------------------------------
   if(!largeMargins) {
      margins <- c(3.25,2.6,3,1) + 0.7   # Margins as c(bottom, left, top, right)
            ...
            if( (length(hideCurves) > 0) && (length(intersect(c(lineNum), hideCurves)) > 0) ) {
               ...
               # ----- Lines or Steps plot ----------------------------------
                  ...
                  if(length(xPlotSet) > 0) {
                     lineWidth <- 1*lineWidthScaleFactor
                     if((length(vLevels) > 1) || (length(wLevels) > 1)) {
                        lineWidth <- 3*lineWidthScaleFactor
                     }
...
# ====== Create plots =======================================================
createPlots <- function(simulationDirectorySet,
                        plotConfigurations,
                        customFilter="",
                        customFormatter=applyFormatter,
                        zColorArray=c(), outputName)
{
   inputDirectorySet <- c()
   inputFileSet      <- c()
   if(!plotOwnOutput) {
      pdf(paste(sep="", outputName, ".pdf"),
   ...
   for(i in 1:length(plotConfigurations)) {
      ...
      resultsNameSet      		<- c()
      simulationDirectorySet 	<- unlist(plotConfiguration[1], recursive="FALSE")
      ...
      for(resultsName in resultsNameSet) {
		 tmpData <- NULL
		 for(simulationDirectory in simulationDirectorySet) {
